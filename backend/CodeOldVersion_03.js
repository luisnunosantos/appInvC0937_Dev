// DateTime: 20260220_1557
// Modo Inventário, antes de adicionar novos campos.
...
function doPost(e) {
  try {
    ...
    if (params.action === "saveInventoryBatch") {
      ...
      // Converte os objetos JSON num array de arrays para injeção rápida
      const dataToAppend = items.map((item) => [
        item.setNumber,
        item.quantity,
        item.user,
        item.timestamp,
